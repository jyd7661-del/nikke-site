...
| `analyzeSkillTriggers.mjs` | **분류 안 됨 185절** (래칫. 2026-09-24 길티 : 마이티 바니의 바니 모드 상태 조건 +6) | 스킬 절을 **발동 빈도 계열**로 분류한다 — 전투 시작 1회 / 풀버스트 사이클마다 / 평타 N발마다 / 탄창마다 / 풀차지마다 등. 계수를 그냥 더하면 "평타마다 3%"와 "버스트마다 2808%"가 같은 자리에 들어가 **모더니아가 3점**이 된다. 새 캐릭터가 새 표현을 들고 오면 분류 안 된 절이 늘어 ERROR |
...
| 스크립트 | 기준선 | 무엇을 보는가 |
|---|---|---|
| `checkCanonical.mjs` | **ERROR 0 / 621개**(번역 203×3 · 한국어 전용 4×3) | 사이트맵에 실리는 주소마다 자기 자신을 가리키는 canonical·noindex·hreflang이 맞는가 |
