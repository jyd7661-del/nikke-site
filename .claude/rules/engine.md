...
- **아키타입 완전일치는 부분일치보다 항상 강한 근거다**(2026-08-25). 정렬에서 앞에 두고,
  부분일치 점수는 `archetypePartialPoints()`가 완전일치를 천장으로 잡는다. 고치기 전에는
  4/5 부분일치(20점)가 완전일치(14점)를 넘겨 **완전일치 조합 이름이 근거의 73.2%에서
  사라졌다.** 이 점수는 게시판 "AI 점수" 배지로도 보이므로 문장만 고쳐선 안 끝난다.
