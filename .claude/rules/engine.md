...
- 🔴 **정렬·선택에는 반드시 마지막 동점 처리를 둔다**(2026-09-18). 안정 정렬이라 동점이면 배열 순서가
  이기는데, 그 배열은 **사용자가 니케를 고른 순서**다. 실제로 `scoreTeam`이 같은 5명에 25점과 30점을
  내고 있었고(낭비 1명 vs 0명), 40건 중 17건이 순서에 따라 다른 조합을 추천했다. 점수가 아니라
  **정렬 키**(`id` 비교)로 고친다 — 순위를 안 뒤집으니 근거 없는 가중치가 아니다.
  `scripts/testEngineDeterminism.mjs`가 지킨다. 새 정렬을 넣으면 여기에도 표본을 추가할 것.
