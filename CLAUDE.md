...
> 엔진을 넘는다**(`scripts/experimentAiTeams.mjs --variant=tier`, API 키 없이 서브에이전트로도 돈다).
> ⚠️ **2026-09-21 같은 12문항을 API `--live`로 대조하니 73.4%·완전일치 1**이었다 — 엔진과 비슷한 수준이지 "크게 넘는다"가 아니다(1회 표본).
> 얇은 로스터 A/B 40건 **AI 21 : 엔진 9**(판정=클로드, 유저 위임) → 운영 코드 투입. 남은 것은 셰도우 실측 — `docs/open-items.md`.
> 판단 기준도 유저가 정했다: **애드센스가 아니라 "유저가 유용하다고 느끼는가".**
...
- **서버 컴포넌트라 클라이언트 i18n을 못 쓴다**(`app/[lang]/privacy`와 같은 제약). 한국어로 렌더하되
