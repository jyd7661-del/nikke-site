...
engineSrc
  ...
  .split('\n')
  .forEach((raw, i) => {
    // ⚠️ 줄 끝 `\r`를 먼저 뗀다 (2026-09-18).
    //    이 저장소의 작업 사본은 CRLF다(git이 커밋 때 LF로 정규화한다). 그런데 자바스크립트
    //    정규식의 `.`은 **줄바꿈 문자를 안 먹고, `\r`도 줄바꿈 문자다.** 그래서 아래
    //    `.*$`가 `\r` 앞에서 멈추고 `$`(문자열 끝)에 닿지 못해 **줄 끝 주석이 하나도 안 걷혔다.**
    //    결과: 한국어 주석이 달린 코드 줄 13개가 전부 "엔진 코드에 한국어 문자열"로 잡히는 오탐.
    //    LF 사본에서는 통과하고 CRLF 사본에서만 실패해서, 같은 코드가 사람마다 다른 결과를 냈다.
    //    원칙 4 — "오탐이 나는 검사는 아무도 믿지 않는다".
