 ...
 *
 * ■ 판정 규칙 — 짧은 칸은 "글자가 하나라도", 본문은 "비율"
 *   이름처럼 짧은 칸은 그 언어 글자가 **하나라도** 있으면 통과시킨다. 비율로 재면
 *   고유명사가 긴 항목(`Blaze-ing Through Campaign ft. Rapi (라피 축의 캠페인 조합)`)이
 *   전부 걸려 아무도 안 믿는 검사가 된다.
 *
 *   ⚠️ 그런데 **본문(note)에 그 규칙을 쓰면 크게 샌다.** 2026-08-25에 실제로 놓쳤다 —
 *      영어 본문 1,064자짜리에 `[다른 출처의 같은 조합 설명 — …]`, `(속성: Water)` 같은
 *      한국어 표식 몇 글자만 붙어 있어 "한글 있음"으로 통과했다. 그렇게 260건이 빠졌다.
 *      그래서 note는 **영문 글자 비율이 절반을 넘으면 미번역**으로 센다.
 *      실측 분포가 0~20%(190건)와 80~100%(260건)로 깨끗이 갈려 있어 50% 경계에서
 *      오탐이 나지 않는다(40~60% 구간이 0건이다).
 *
...
const EXPECTED = {
  ...
  // 2026-09-13 신캐 2명 추가로 둘 다 +3. 둘 다 **출처가 막혀 있는 것**이지 우리가 빠뜨린 게 아니다:
  //   desc_kr +3 drake-great-villain — 나무위키 문서가 틀만 있고 스킬 칸이 편집용 자리표시자다
...
syn.forEach((a) => {
  ...
  ['name', 'note'].forEach((f) => {
    // name은 "한글이 하나라도", note는 "영문이 절반 미만"으로 본다(위 주석 참고).
    if (nz(a[f]) && (f === 'note' ? MOSTLY_LATIN(a[f]) : !HAS_KO(a[f]))) mark(`archetype.${f}(ko)`, who);
