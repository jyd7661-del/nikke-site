 ...
 *   그게 바로 "조용한 누락"이 된다.
  *
 * ⚠️ **이 수집기는 `desc`(영문)를 덮어쓴다. 그런데 영문 원문에는 숫자 오타가 있다.**
 *    2026-09-07에 한국어·일본어와 대조해 11곳을 고쳤다(소다 321.8→321.28, 미카: 스노우 버디
 *    39.3→39.93 등. 그중 6곳은 조합 비교기 계산에 실제로 들어간다).
 *    이 수집기를 다시 돌리면 **그 오타가 되돌아온다.**
 *    다만 `scripts/testSkillSources.mjs`가 기준선 0으로 잠가 놨으므로 `/verify`가 잡는다 —
 *    걸리면 그 스크립트가 찍어 주는 목록대로 다시 고칠 것.
 */
