      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        {SORTED.map((c) => (
          <Link
            key={c.id}
            href={`/nikke/${c.id}`}
            className="rounded-xl bg-slate-800/40 hover:bg-slate-800 p-3 transition-colors"
          >
            {c.img && (
              ...
              <img
                ...
                alt={c.name_kr}
                loading="lazy"
                className="w-full aspect-[3/4] object-cover rounded-lg bg-slate-800 mb-2"
              />
            )}
            <div className="font-bold text-slate-100 text-sm leading-tight">{c.name_kr}</div>
            <div className="text-xs text-slate-500 mt-1">
              B{c.burst} · {ELEMENT_KR[c.element] || c.element} · {CLASS_KR[c.class] || c.class}
            </div>
            <div className="text-xs text-slate-600">{CORP_KR[c.manufacturer] || c.manufacturer}</div>
          </Link>
        ))}
      </div>
    </main>
