...
export function generateMetadata({ params }) {
  ...
  return {
    ...
    ...localeMeta(params.lang, `/guide/${g.slug}`),
    openGraph: {
      type: 'article',
      title: g.title,
      description: g.description,
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10">
      ...
      <nav className="text-sm text-slate-500">
        <Link href={localePath(params.lang, '/guide')} className="hover:text-slate-300">← 가이드 목록</Link>
      </nav>
      ...
      <footer className="mt-12 pt-6 border-t border-slate-800 text-sm text-slate-500 space-y-3">
        ...
        <p>
          <Link href={localePath(params.lang, '/')} className="text-sky-400 hover:underline">보유 니케로 조합 추천받기</Link>
          {' · '}
          <Link href={localePath(params.lang, '/nikke')} className="text-sky-400 hover:underline">캐릭터 도감</Link>
          {' · '}
          <Link href={localePath(params.lang, '/guide')} className="text-sky-400 hover:underline">다른 가이드</Link>
        </p>
      </footer>
    </main>
  );
