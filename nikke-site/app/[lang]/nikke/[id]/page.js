...
import { seo, dexLabels } from '@/lib/seoText';
...
export function generateMetadata({ params }) {
  ...
  // 언어별 주소(2026-09-19): 세 언어판 모두 색인. 한국어 제목·설명은 도입 전과 글자까지 같다
  // (lib/seoText.js). **한 주소 = 한 언어** — 9-16에 한 주소의 제목에 세 언어를 섞었다가 한국어 검색이 떨어졌다.
  const S = seo(params.lang);
  return {
    title: S.dexTitle(c),
    description: S.dexDesc(c, dexLabels(c, params.lang), t, teams.length),
    ...localeMeta(params.lang, `/nikke/${c.id}`),
    openGraph: {
      title: S.dexOgTitle(c),
