...
AI 추천은 조합을 AI가 만드는 게 아니라, `lib/synergyEngine.js`가 확정한 5인 조합을 **설명하는 문장만**
AI에게 맡깁니다. 그 엔진은 완전히 결정적이라 같은 보유 캐릭터·모드·애장품이면 항상 같은 결과가 나오고,
따라서 AI에게 보내는 입력도 완전히 같습니다. 캐시가 없으면 같은 답을 매번 돈 주고 다시 받게 됩니다.

- **DB 마이그레이션 필요**: `supabase/ai_explain_cache_migration.sql`을 Supabase SQL Editor에서 한 번
  실행하세요. 실행하지 않아도 사이트는 정상 동작하지만(캐시 실패는 조용히 무시하고 새로 생성) 비용
  절감 효과가 없습니다.
- **캐시 키**는 로스터가 아니라 *AI에게 실제로 보낸 입력*(모드/언어/멤버/근거 문장/아키타입 노트)의
  SHA-256입니다. 그래서 서로 다른 로스터라도 결론이 같으면 캐시를 공유하고, 반대로 엔진이나 데이터가
  바뀌어 근거 문장이 달라지면 키가 자동으로 바뀌어 새 설명이 생성됩니다 — 수동 무효화가 필요 없습니다.
  프롬프트를 크게 바꿔 기존 캐시를 통째로 버리고 싶을 때만 `buildCacheKey`의 `v` 값을 올리세요.
- **캐시 적중은 하루 사용 횟수(`DAILY_LIMIT`)를 깎지 않습니다.** 비용이 들지 않는 요청이기 때문입니다.
- **AI 호출이 실패했을 때의 대체 문장은 캐시에 저장하지 않습니다.** 저장하면 그 조합에 열화된 설명이
  영구히 박힙니다.
