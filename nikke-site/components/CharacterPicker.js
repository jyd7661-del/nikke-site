...
export default function CharacterPicker({ ownedIds, treasureIds, onToggle, onToggleTreasure, onClear, aiMode }) {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    cardRefs.current.forEach((el, id) => {
      ...
      //        (추천 결과 패널이 선택 그리드 **아래**에 있어서다).
      //        한때 168장 전부가 -1500px 이동으로 찍힌 적이 있는데, 그건 브라우저 패널이
      //        표시되지 않아 프레임을 안 그리던 상태의 허상이었다. 이 경로는 실측으로
      //        확인된 버그가 아니라 **구조상 남아 있던 가능성**이고, 컨테이너 기준으로
      //        바꾸면 레이아웃이 어떻게 바뀌든 애초에 성립하지 않는다.
      //
