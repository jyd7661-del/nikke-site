...
그 상태에서 `checkCanonical`을 돌리면 **203건 전부 "빌드 산출물 없음" ERROR**가 난다.

회귀처럼 보이지만 아니다. `next build`를 다시 하면 `ERROR 0`으로 돌아온다.
화면을 확인하려고 dev를 켠 뒤에 검사를 돌리는 흐름에서 걸리기 쉽다.

## Git Bash가 경로처럼 생긴 인자를 Windows 경로로 바꾼다 (2026-08-27)

MSYS는 `/`로 시작하는 인자를 Windows 경로로 변환한다. `curl -w "$p -> %{http_code}"` 에서
`$p`가 `/nikke/crown`이면 `C:/Program Files/Git/nikke/crown`으로 바뀌어 출력된다.

⚠️ **JSON 본문(`{"path":"/nikke/crown"}`)은 `{`로 시작해서 변환되지 않는다.** 그래서
"표시는 깨졌는데 실제 요청은 정상"인 상태가 나오고, 이것을 요청 실패로 오해하기 쉽다.
막으려면 `export MSYS_NO_PATHCONV=1`.
